...
/** Bands from least to most severe — the ordering used to compare bands. */
export const BAND_ORDER: SeverityBand[] = [
  "normal",
  "ringan",
  "sederhana",
  "teruk",
...
/** The most severe of the three subscale bands. */
export function worstBand(scores: ScreeningScores): SeverityBand {
  return [
    scores.stressBand,
    scores.anxietyBand,
    scores.depressionBand,
  ].reduce((worst, b) =>
...
/**
 * Whether to offer the respondent a counsellor appointment: any subscale at
 * `sederhana` or above, OR the crisis flag — the latter catches a low total
 * score that still hides a hopelessness/worthlessness answer.
 */
export function needsCounselorOffer(scores: ScreeningScores): boolean {
  return (
    scores.crisisFlag ||
    BAND_ORDER.indexOf(worstBand(scores)) >= BAND_ORDER.indexOf("sederhana")
  );
}
