...
| `counseling_referrals` | Voluntary appointment requests (identity + score snapshot) | Insert-only (anon) |

**RLS Enforces:** Anon can INSERT but not SELECT; only registered counselors can READ data.
A respondent can therefore submit their own referral but can never read anyone's
contact details back out — not even their own.
...
✅ **Anonymous by Default:** The screening itself collects no names, IDs, or identifiers  
✅ **Opt-In Referral Only:** When a screening reaches Sederhana or above (or trips the crisis
flag), the respondent is *offered* a counsellor appointment. Identity is collected only if
they choose "Ya", behind a second explicit consent, and is disclosed on the landing page
before the screening begins. Declining is recorded as an anonymous tally with no identity.  
