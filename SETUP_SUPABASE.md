...
The schema lives in `supabase/migrations/`. Apply the files **in numbered order** —
`002_counseling_referrals.sql` depends on the `is_staff()` function created by
`001_init.sql`. You have two options:
...
- [ ] `counseling_referrals` table (empty, waiting for appointment requests)
