...
test("RLS: anon can INSERT a declined counseling_referrals row but cannot SELECT it back", { skip }, async () => {
  const { createClient } = await import("@supabase/supabase-js");
  const supabase = createClient(url!, anonKey!, {
    auth: { persistSession: false },
  });
  ...
  const ins = await supabase.from("counseling_referrals").insert({
